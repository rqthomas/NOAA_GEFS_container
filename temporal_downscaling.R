...
temporal_downscale <- function(input_file, output_file, overwrite = TRUE){
  ...
  forecast_noaa_ds <- forecast_noaa_ds %>% 
    mutate(relative_humidity = humidity::SH2RH(q = forecast_noaa_ds$specific_humidity,
                                               t = forecast_noaa_ds$air_temperature, 
                                               p = forecast_noaa_ds$air_pressure,
                                               isK = TRUE)) %>% 
...
downscale_spline_to_hourly <- function(df,VarNames){
  # --------------------------------------
  ...
  interp.df.days <- seq(min(df$days_since_t0), as.numeric(max(df$days_since_t0)), 1/24)
...
downscale_ShortWave_to_hrly <- function(df,lat, lon){
  ...
  ShortWave.ds <- data.hrly %>% 
    ...
    dplyr::mutate(doy = lubridate::yday(time) + hour/24)%>% 
...
downscale_repeat_6hr_to_hrly <- function(df, varName){
  
  #Get first time point
  t0 <- min(df$time)
  
  df <- df %>% 
    dplyr::select("time", varName) %>% 
